...
class chessBoardWindow(QMainWindow):
    ...
    def checkValidMove(self, fromPos, toPos, rules):
        # Use the appropriate rule from the rules classes to chek the movement of the piece.
        moveIsValid = rules.isPathFree(self, fromPos, toPos,
                                       self.piecePos[fromPos[1]][fromPos[0]].pieceColor, self.piecePos, self.nodeArr)
        # The following line will probably be moved.
        if self.pieceSet[fromPos[1]][fromPos[0]][0] == self.pieceSet[toPos[1]][toPos[0]][0]:
            moveIsValid = False
        return moveIsValid

    def rollCapturePiece(self, attacker, target):
